...
const 길이클래스들: readonly 길이클래스[] = [
  { 이름: 'short', 길이비율: 0.55, 세기: 1.04, 릴리스: 0.04, 윤곽: [1, 0.58, 0.16, 0.02] },
  { 이름: 'medium', 길이비율: 0.72, 세기: 0.96, 릴리스: 0.07, 윤곽: [1, 0.88, 0.27, 0.03] },
  { 이름: 'long', 길이비율: 0.88, 세기: 0.88, 릴리스: 0.1, 윤곽: [1, 0.78, 0.48, 0.06] },
  { 이름: 'held', 길이비율: 0.98, 세기: 0.78, 릴리스: 0.14, 윤곽: [1, 0.83, 0.43, 0.13] },
]
...
function 코드톤점수구하기(표본들: Float32Array, 입력표본율: number, 기호순서: number, 보이싱번호: number): number {
  const 주파수들 = 코드톤주파수들구하기(기호순서, 보이싱번호)
  const 합계 = 주파수들.reduce((누적, 주파수) => 누적 + 괴르첼(표본들, 입력표본율, 주파수), 0)
  return 합계 / Math.sqrt(주파수들.length)
}
...
function 음악기호쓰기(
  ...
  const 길이번호 = (기호 >> 3) & 0b11
  const 당김번호 = (기호 >> 5) & 0b1
  const 슬롯초 = 표본수 / 출력표본율
  const 길이클래스 = 길이클래스들[길이번호]
  const 시작초 = 당김번호 ? 0 : 슬롯초 * 0.08
  const 길이초 = Math.max(0.08, (슬롯초 - 시작초) * 길이클래스.길이비율)
  const 시작 = 위치 + Math.floor(시작초 * 출력표본율)
  const 길이 = Math.max(96, Math.floor(Math.min(길이초, 슬롯초 - 시작초) * 출력표본율))
  const 강세 = 기호강세구하기(기호순서) * 길이클래스.세기
...
function 음표더하기(
  ...
  for (let 순서 = 0; 순서 < 표본수 && 위치 + 순서 < 채널.length; 순서 += 1) {
    if (위치 + 순서 < 0) continue
    const 상대초 = 순서 / 출력표본율
    const 포락선 = 피아노포락선구하기(상대초, 전체초, 릴리스)
...
function 베이스깔기(
  ...
  음표더하기(채널, 위치, 출력표본율, 보조음, 길이, 셀번호 === 0 ? 0.18 : 0.12, 기호순서 * 11, 0.16)
}
...
function 피아노포락선구하기(상대초: number, 전체초: number, 릴리스초: number): number {
  const 어택 = Math.min(1, 상대초 / 0.0035)
  const 감쇠 = 0.1 + 0.9 * Math.exp(-상대초 * 7.4)
...
function 버퍼부드럽게제한하기(채널: Float32Array, 출력표본율: number): void {
  ...
  for (let 순서 = 0; 순서 < 채널.length; 순서 += 1) {
    채널[순서] = Math.tanh(채널[순서] * 1.18) * 0.86
  }
}
...
function 기호세부찾기(
  ...
  for (const 보이싱번호 of 보이싱후보들) {
    for (let 당김번호 = 0; 당김번호 < 2; 당김번호 += 1) {
      for (let 길이번호 = 0; 길이번호 < 길이클래스들.length; 길이번호 += 1) {
        const 후보 = (당김번호 << 5) | (길이번호 << 3) | 보이싱번호
        const 점수 = 참조점수구하기(표본들, 입력표본율, 기호순서, 후보)
        if (점수 > 최고점수) {
          최고점수 = 점수
          최고기호 = 후보
        }
...
function 참조점수구하기(표본들: Float32Array, 입력표본율: number, 기호순서: number, 후보: number): number {
  const 참조 = new Float32Array(표본들.length)
  음악기호쓰기(참조, 0, 입력표본율, 후보, 표본들.length, 기호순서)
  let 곱합 = 0
  let 참조합 = 0
...
function 보이싱미디음들구하기(기호순서: number, 보이싱번호: number): number[] {
  ...
  return 가까운배치하기(간격들.map((간격) => 근음 + 간격))
}
...
function 가까운배치하기(기본음들: number[]): number[] {
  ...
  const 음들 = 기본음들
    .map((음, 인덱스) => {
      let 후보 = 음
      const 목표 = 다섯음보이싱 ? 50 + 인덱스 * 7 : 57 + 인덱스 * 5
      while (후보 - 목표 > 6) 후보 -= 12
      while (목표 - 후보 > 6) 후보 += 12
      while (후보 < (다섯음보이싱 ? 43 : 49)) 후보 += 12
      while (후보 > (다섯음보이싱 ? 84 : 77)) 후보 -= 12
      return 후보
    })
    .sort((왼쪽, 오른쪽) => 왼쪽 - 오른쪽)
  ...
  return 음들.map((음) => (음 > (다섯음보이싱 ? 86 : 79) ? 음 - 12 : 음))
}
