...
Jazzmodem은 아직 실험 단계입니다. 현재 MVP는 재즈 블루스 진행 위에 보이싱, 음 길이, 당김 타이밍을 함께 싣는 음악 모뎀을 중심으로 구현되어 있습니다.

- 송신/수신 단일 페이지
- 재즈 피아노풍 합성음 기반 기호 전송
- 138BPM 스윙 8분 그리드와 12마디 재즈 블루스/ii-V 턴어라운드 기반 코드 진행
- 보이싱 3비트 + 길이 2비트 + 당김 1비트 음악 기호
...
Jazzmodem은 단순 비프음 대신 음악적으로 들리는 전송음을 목표로 합니다. 현재 구현은 138BPM 스윙 8분 그리드, 12마디 재즈 블루스, ii-V 턴어라운드의 루트리스 피아노 보이싱을 따라가며, 각 음악 기호에 보이싱 3비트, 음 길이 2비트, 당김 타이밍 1비트를 함께 실어 보냅니다.
...
현재 테스트는 CRC, 음악 기호 왕복, 깨진 청크 검출, 합성 오디오 복조, 짧은 텍스트 전체 왕복을 검증합니다.
